...
**Fase actual: agente de consulta y display, no de acción.** Wattson lee los datos de las fuentes de la empresa, responde preguntas sobre ellos, y los muestra de forma clara y linda (dashboards + reportes). No escribe ni muta nada.
...
```
Fuentes (Notion, etc.) → sync programado → store propio (Postgres) → Wattson consulta el store → display (dashboards + morning report)
```

- Wattson **no** consulta las fuentes en vivo para los dashboards. Consulta el store ya sincronizado. Razones: rate limits de las fuentes (Notion corta a ~180 req/min) y poder correr agregaciones de verdad.
...
- El store propio (Postgres) es a la vez el **warehouse de consulta** y la **memoria/auditoría** de Wattson. Las fuentes (Notion, etc.) NO son el datastore de Wattson.
...
- **Notion NO es el datastore de Wattson.** Sincronízalo a Postgres.
...
- Para que un cambio en Notion *dispare* algo en Wattson: **webhooks** de la API (el MCP no tiene superficie de eventos).
- **Least-privilege gratis:** las integraciones de Notion solo ven lo que les compartes explícitamente. Dale a Wattson acceso únicamente a los espacios/DBs que necesita.

---

## 8.1 Interfaz — nota sobre WhatsApp (canal confirmado)

WhatsApp es la superficie de **conversación** (1:1 con el número de Wattson) y de **push** (morning report). Mismo patrón headless que Notion: **API REST + webhooks**, no MCP interactivo. WhatsApp recibe por webhook → Wattson procesa → responde por API.
...
- **Setup:** Meta Business verificada + número **dedicado** a Wattson (no personal) + alta en WhatsApp Business Platform.
...
- Wattson es un servicio **always-on, server-side**. Vive en cloud/PaaS (Railway, Render, Fly) o una VM — o en la infra de la firma si los datos no pueden salir. Debe estar prendido siempre porque el sync y el morning report son programados.
...
- **L0 (lectura/Q&A):** casi todo Wattson en esta fase.
...
- ❌ Usar Notion (u otra fuente) como datastore de Wattson.
...
- ❌ Asistente de escritorio / control local — Wattson es server-side, always-on.
